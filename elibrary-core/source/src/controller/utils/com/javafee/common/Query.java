...
public class Query {
	...
	public enum TabOutboxPageEventQuery {
		...
		MESSAGE_BY_SENDER_LOGIN(
				"from Message mes where mes.sender.login = ?0 and (mes.isDraft is null or mes.isDraft is false)"), //
		DRAFT_MESSAGE_BY_SENDER_LOGIN("from Message mes where mes.sender.login = ?0 and mes.isDraft is true"), //
		...
		DISTINCT_MESSAGE_BY_RECIPIENT_USER_DATA_AND_SENDER_LOGIN(
				"select distinct mes from Message mes join fetch mes.recipient r where r.userData = ?0 and mes.sender.login = ?1 " //
						+ "and mes.isDraft is null or mes.isDraft is false"), //
