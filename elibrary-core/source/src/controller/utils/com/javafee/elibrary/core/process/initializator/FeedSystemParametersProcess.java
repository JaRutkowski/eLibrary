	...
	private void feedSystemParameters() {
		if (!Common.checkIfAnySystemParametersExists()) {
			...
			HibernateUtil.getEntityManager().createNativeQuery(Query.ProcessQuery.FEED_SYSTEM_PARAMETERS_DATA.getValue())
					...
					.setParameter(18, Constants.DATA_BASE_SYSTEM_PARAMETER_APPLICATION_NUMBER_OF_ATTEMPTS_LIMIT_NAME)
					.setParameter(19, Constants.DATA_BASE_SYSTEM_PARAMETER_APPLICATION_NUMBER_OF_ATTEMPTS_LIMIT_VALUE)
					.setParameter(20, Constants.DATA_BASE_SYSTEM_PARAMETER_APPLICATION_BLOCK_ACCOUNT_FUNCTIONALITY_NAME)
					.setParameter(21, Constants.DATA_BASE_SYSTEM_PARAMETER_APPLICATION_BLOCK_ACCOUNT_FUNCTIONALITY_VALUE)
					.executeUpdate();
