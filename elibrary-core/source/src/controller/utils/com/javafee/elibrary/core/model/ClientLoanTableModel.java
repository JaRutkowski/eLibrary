...
public class ClientLoanTableModel extends LoanTableModel {
	public ClientLoanTableModel() {
		super();
		this.columns = new String[]{
				...
				SystemProperties.getInstance().getResourceBundle().getString("loanTableModel.isProlongatedCol")};
	}
	...
	protected void prepareHibernateDao() {
		...
		setLends(HibernateUtil.getSession().createQuery("from Lend as len join fetch len.volume " +
				"where len.isReturned = false and len.client = " + loggedUserId).list());
	}
	...
	public Object getValueAt(int row, int col) {
		...
		switch (Constants.ClientLoanTableColumn.getByNumber(col)) {
			...
			case COL_IS_PROLONGATED:
				return checkIfProlongated(lend.getLendDate(), lend.getReturnedDate())
	...
	private boolean checkIfProlongated(Date lendDate, Date returnedDate) {
		return Common.diffDatesByPeriodType(lendDate, returnedDate, Constants.APPLICATION_PROLONGATION_PERIOD.getFirst())
				> Constants.APPLICATION_PROLONGATION_PERIOD.getSecond();
	}
}